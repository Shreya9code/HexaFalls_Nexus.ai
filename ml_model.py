...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1:
        ...
        if os.path.exists(video_path):
            ...
            result = analyze_facial_expressions(video_path)
            ...
            if "error" not in result:
                ...
                api_result = {
                    "speechAnalysis": {
                        "text": "Video analysis completed successfully",
                        "evaluation": "POSITIVE",
                        "confidence": 0.85,
                        "feedback": result.get("facial_expression_feedback", "Good facial expressions detected."),
                        "suggestions": result.get("facial_suggestions", ["Keep up the good work!"])
                    },
                    "facialAnalysis": {
                        "dominantExpression": "confident",
                        "feedback": result.get("facial_expression_feedback", "You appeared confident and positive throughout the interview.")
                    },
                    "isComplete": True,
                    "facial_metrics": result.get("facial_analysis", {})
                }
